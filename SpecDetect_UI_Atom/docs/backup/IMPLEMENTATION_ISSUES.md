...
### Issue #1: RMCP Checksum 插入位置错误 (2026-04-18 标记为已忽略)

**状态**: ⚠️ 标记忽略
**原因**: emulated_atom 使用 rmcp_proxy 转发 RMCPTP 帧，checksum 由 rmcp_proxy 计算，不受 emulated_atom 影响

---

### Issue #2: SOAP 请求使用 UTF-8 编码 (2026-04-18 标记为已忽略)

**状态**: ⚠️ 标记忽略
**原因**: emulated_atom 使用 gb2312 编码，正确。test_fscan529.py 是独立测试脚本，不影响主流程。

---

### Issue #3: FSCAN-434 只解析不生成 (2026-04-18 标记为已忽略)

**状态**: ⚠️ 标记忽略
**原因**: Real Atom 实际只发送 FSCAN-529 帧，不需要生成 FSCAN-434

---

### Issue #4: 回调日志用模拟数据 (2026-04-18 标记为已忽略)

**状态**: ⚠️ 标记忽略
**原因**: USE_MOCK_DATA 标志已正确处理，rmcp_proxy 直连路径已验证
...
- [x] Issue #1: 抓包验证设备是否因校验和错误拒绝请求 → 已忽略，rmcp_proxy处理
- [x] Issue #2: 用正确编码的请求测试设备响应 → 已忽略，emulated_atom正确
- [x] Issue #3: 确认真实设备是否会发送 FSCAN-434 → 已忽略，Real Atom不发送
- [x] Issue #4: rmcp_proxy 直连路径的完整性 → 已忽略，已验证
...
| 优先级 | Issue | 状态 |
|--------|-------|------|
| P0 | #1 Checksum | ⚠️ 已忽略 (rmcp_proxy处理) |
| P0 | #2 UTF-8编码 | ⚠️ 已忽略 (emulated_atom正确) |
| P1 | #3 FSCAN-434 | ⚠️ 已忽略 (不需生成) |
| P2 | #4 回调日志 | ⚠️ 已忽略 (已验证) |
