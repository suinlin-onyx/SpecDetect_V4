...
def receive_streamsrc():
    ...
    while recv_count < 30:  # 最多收30帧
        try:
            # 接收完整帧 (1086 字节)
            frame = b''
            while len(frame) < 1086:
                chunk = sock.recv(1086 - len(frame))
                if not chunk:
                    break
                frame += chunk
            ...
            # Offset 18-19: PL (2 bytes, big-endian)
            pl = struct.unpack('>H', frame[18:20])[0]
            # Offset 20-21: EL (2 bytes)
            # Offset 22-23: PAD (2 bytes)
            # Offset 24: DT (1 byte)
            dt = frame[24]
            # Offset 25-28: DL (4 bytes, little-endian)
            dl = struct.unpack('<I', frame[25:29])[0]

            print(f"[STREAM] 帧{recv_count}: leader=0x{leader:08X}, ver={ver}, stc={stc}, dt={dt}, dl={dl}, pl={pl}")

            # 解析频谱数据 (从 offset 62 开始)
