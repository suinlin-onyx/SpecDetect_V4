...
## 五、streamsrc vs rmcp 数据对比

**配对数据**: streamsrc `streamsrc_raw_20260416_101328.log` ↔ rmcp `raw_fscan_20260416_101230.log`（同一时段，10:13:29）

| 属性 | streamsrc FSCAN-529 | rmcp FSCAN |
|------|---------------------|-----------|
| 总 buffer | 529 int16/帧 | 512 电平/帧 |
| 有效频谱点 | **379**（去元数据后） | **512** |
| 压缩比 | ~3.8:1 vs rmcp | 1:1 |
| 覆盖范围 | ~9.5 MHz | 36 MHz |
| 帧速率 | ~3 Hz | ~3 Hz |
| 精度 | ~16 级量化 | ~700 级 |
| dBm 范围 | [-75.8, -28.7] | [-98.2, -29.6] |

**结论**: streamsrc 是 rmcp 的**实时压缩监测版本**，同源于 Device 信号但用途不同：
- rmcp: 完整存档（GWJ004 格式，512点/36MHz）
- streamsrc: 实时告警（Atom 私有压缩，379点/局部频段）
